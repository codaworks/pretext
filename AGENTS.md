...
- Segment metrics cache is `Map<font, Map<segment, metrics>>`; shared across texts and resettable via `clearCache()`. Width is only one cached fact now; grapheme widths and other segment-derived facts can be populated lazily. With `fontFeatureSettings`, the outer key widens to `font + features` so feature widths never leak into plain measurement.
- Canvas 2D contexts expose no font-feature control; feature measurement rides the CSS `font-feature-settings` of hidden in-document canvas elements (one per feature string, capped pool, released by `clearCache()`). OffscreenCanvas has no such channel, so featureless measurement keeps the OffscreenCanvas default context and feature requests without a document are silently ignored.
- Segment widths are measured with a short following context as shaping lookahead (`measure(seg + ctx) − measure(ctx)`), because browsers shape whole runs and cross-boundary kerning/required ligatures change a segment's advance. The context takes one grapheme per following visible segment and walks through spaces until it includes a non-space grapheme — a trailing space alone is not enough lookahead for ligature rules that skip spaces and match on the next letter. Pair strings ride the same per-font cache; tabs, soft hyphens, zero-width breaks, hard breaks, and CJK sub-units contribute no context.
